...
def upload_stock_data(excel_file_path):
    ...
    for r_idx in range(3, len(df)):
        ...
        if not product_name or product_name.startswith('TOTAL') or product_name.startswith('STOCK AS AT') or product_name.startswith('PRODUCTS'):
        ...
        if pd.notna(unit_value_raw) and str(unit_value_raw).strip() != '' and str(unit_value_raw).strip() != '[object Object]':
            try:
                unit_value = Decimal(str(unit_value_raw).replace(',', ''))
            except (ValueError, TypeError):
                print(f"Could not parse unit value for product {product_name}: {unit_value_raw}. Defaulting to 0.00")
                unit_value = Decimal('0.00')
        else:
            print(f"Invalid or missing unit value for product {product_name}: {unit_value_raw}. Defaulting to 0.00")

        for i, branch_name in enumerate(branch_names):
            if not branch_name or pd.isna(branch_name):
                continue

            quantity_str = row_data[i + 1] # Quantity for each branch starts from column B (index 1)
            try:
                quantity = int(quantity_str) if pd.notna(quantity_str) and str(quantity_str).strip() != '' else 0
            except (ValueError, TypeError):
            ...
            if not created:
                ...
                stock_item.save()
                print(f"Updated stock for {product_name} at {branch_name}. New quantity: {stock_item.quantity}")
            else:
                print(f"Created new stock item: {product_name} at {branch_name} with quantity {quantity}")
