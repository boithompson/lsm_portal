...
from workshop.forms import VehicleForm, JobSheetForm, InternalEstimateForm, EstimatePartForm
...
def create_internal_estimate(request, vehicle_id):
    vehicle = Vehicle.objects.get(id=vehicle_id)
    EstimatePartFormSet = inlineformset_factory(InternalEstimate, EstimatePart, form=EstimatePartForm, extra=1, can_delete=True)
    if request.method == 'POST':
